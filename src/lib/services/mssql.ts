...
export async function mssqlQuery(connectionId: string, sql: string): Promise<MssqlQueryResult> {
  try {
...
export async function mssqlExecute(connectionId: string, sql: string): Promise<MssqlQueryResult> {
  try {
    const result = await invoke<{
      columns: string[];
      rows: Record<string, unknown>[];

...
export function parseSql(sql: string, options?: ParseSqlOptions): ParsedQuery | null {
  ...
  try {
    ...
    if (stmt.from && Array.isArray(stmt.from)) {
      for (const fromItem of stmt.from) {
        ...
        if ("table" in fromItem && fromItem.table) {
          ...
          if (
            validTableNames !== null &&
            !validTableNames.includes(tableName) &&
            !cteNameSet.has(tableName)
          ) {
            continue;
          }
    ...
    const filters = stmt.where
      ? parseWhereClause(stmt.where, tableAliasMap, tables, subqueries, cteNameSet, validTableNames)
      : [];
...
function resolveTableName(
  nameOrAlias: string | { type?: string; value?: string } | undefined,
...
function parseWhereRecursive(
  ...
  if (e.type === "binary_expr") {
    ...
    if (left?.type === "column_ref") {
      const columnName = typeof left.column === "string" ? left.column : left.column?.expr?.value;
      ...
      if (columnName) {
        ...
        } else if (rightAsExprList?.type === "expr_list" && rightAsExprList.value?.[0]?.ast) {
          // IN (SELECT ...) case
...
function parseSubqueryAst(
  ...
  const effectiveValidTableNames =
    validTableNames === undefined ? getTableNames() : validTableNames;
  const tables: ParsedTable[] = [];
  const joins: ParsedJoin[] = [];
  const selectAggregates: ParsedSelectAggregate[] = [];
  const columnAggregates: ParsedColumnAggregate[] = [];
  ...
  if (stmt.from && Array.isArray(stmt.from)) {
    for (const fromItem of stmt.from) {
      ...
      // Check for derived table (FROM (SELECT ...) AS alias)
      if (fi.expr?.ast) {
        const derivedTableParsed = parseSubqueryAst(
          fi.expr.ast,
          cteNameSet,
          effectiveValidTableNames,
        );
        if (derivedTableParsed) {
          const subqueryIndex = subqueries.length;
          subqueries.push({
      ...
      if (fi.table) {
        ...
        if (
          effectiveValidTableNames !== null &&
          !effectiveValidTableNames.includes(tableName) &&
          !cteNameSet?.has(tableName)
        ) {
          continue;
        }
    ...
    for (const col of stmt.columns) {
      if (col === "*") {
        ...
        if (colObj.expr?.type === "column_ref" && colObj.expr.column === "*") {
          ...
          const columnName =
            typeof colObj.expr.column === "string"
              ? colObj.expr.column
      ...
      if (g.type === "column_ref") {
        ...
        if (columnName) {
          const tableName = g.table
            ? resolveTableName(g.table, tableAliasMap)
            : findTableForColumn(tables, columnName)?.tableName;
  ...
  if (stmt.orderby && Array.isArray(stmt.orderby)) {
    for (const order of stmt.orderby) {
      const o = order as {
        expr?: { type?: string; table?: string; column?: string | { expr?: { value?: string } } };
        type?: string;
      };
      if (o.expr?.type === "column_ref") {
        const columnName =
          typeof o.expr.column === "string" ? o.expr.column : o.expr.column?.expr?.value;
        if (columnName) {
          const tableName = o.expr.table
            ? resolveTableName(o.expr.table, tableAliasMap)
            : findTableForColumn(tables, columnName)?.tableName;
          const fullColumn = tableName ? `${tableName}.${columnName}` : columnName;
          orderBy.push({
            column: fullColumn,
